...
public class ExampleServiceTracker extends ServiceTracker {
    ...
    public Object addingService(ServiceReference reference) {
        ...
        System.out.println("WebApp2 got new ExampleService (" + exampleService.getName() + "). Going add it to list");
        exampleServices.add(exampleService);
        return context.getService(reference);
    }
    
    public void removedService(ServiceReference reference, Object service) {
        System.out.println("WebApp2 got ExampleService (" + ((ExampleService)service).getName() + ") be removed. Going remove it to list");
